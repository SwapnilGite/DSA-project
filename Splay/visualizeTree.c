...
void displayTree(sTree root)
{
    ...
    else
    {
        ...
        fprintf(fileOp, "digraph splayTree\n{\n");
        // Creating the Label at the Bottom
        fprintf(fileOp, "label = \"IP Network using Splay Tree\"\n");
        ...
        while (front != NULL)
        {
            sTree current = dequeue(&front, &rear);
        ...
        system("dot -Tpng temp.dot -o Splay.png");
        system("BST.png");
    }
}

// preorder traversal
void preorderVisual(FILE *fileOp, sTree t)
{
    if (t != NULL)
    {
